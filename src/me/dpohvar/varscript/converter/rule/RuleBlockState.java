...
public class RuleBlockState extends ConvertRule<BlockState>{
    ...
    @Override
    public <V> BlockState convert(V object, me.dpohvar.varscript.vs.Thread thread,Scope scope) throws NextRule {
        if (object==null) return null;
        if (object instanceof Block) return ((Block)object).getState();
        if (object instanceof Number) return thread.getProgram().getCaller().getLocation().add(0.0,((Number)object).doubleValue(),0).getBlock().getState();
        if (object instanceof Character) return thread.getProgram().getCaller().getLocation().add(0.0,((Character)object),0).getBlock().getState();
        if (object instanceof String) return Bukkit.getPlayer(((String)object).trim()).getLocation().getBlock().getState();
        if (object instanceof Entity) return ((Entity)object).getLocation().getBlock().getState();
        if (object instanceof Vector) return thread.getProgram().getCaller().getLocation().add((Vector)object).getBlock().getState();
