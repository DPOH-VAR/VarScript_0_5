...
import org.dpohvar.varscript.vs.Command;
import org.dpohvar.varscript.vs.CommandDebug;
...
import org.dpohvar.varscript.vs.init.*;
...
public class VSCompiler {
    ...
    public static void addRule(CompileRule rule) {
        if (validatorDescription.contains(rule.getDescription()))
            throw new RuntimeException(rule.toString() + " DESC NOT VALID!!!!");
        validatorDescription.add(rule.getDescription());
        if (validatorName.contains(rule.toString()))
            throw new RuntimeException(rule.toString() + " NAME NOT VALID!!!!");
    ...
    private static void addWorkerToRules(Worker worker) {
        ...
        for (byte b : sBytes) {
            ...
            if (current[b & 0xFF] instanceof Object[]) current = (Object[]) current[b & 0xFF];
                ...
                throw new RuntimeException("can not add worker to rules: rule already exists " + current[b & 0xFF] + " " + worker);
        }
        if (current[id & 0xFF] != null)
            throw new RuntimeException("can not add worker to rules: worker already exists " + current[id & 0xFF]);
    ...
    public static CommandList compile(String name, CompileSession compileSession, boolean first) throws SourceException {
        ...
        try {
            read_operands:
            while (functionSession.hasOperand()) {
